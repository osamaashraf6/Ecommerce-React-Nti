  ...
  async (
    ...
    if (!cart) {
      ...
      const productIndex: number = cart.items.findIndex(
        (item: IItems) => item.productId._id!.toString() === req.body.productId
      );
      if (productIndex > -1) {
...
// ! to let the user determine the quantity only
// export const createOneProductAtCart = asyncHandler(
//   async (
//     req: express.Request,
//     res: express.Response,
//     next: express.NextFunction
//   ): Promise<void> => {
//     const { productId, color, size } = req.body; // Extract color and size from request
...
//     // 2. Check if the user has a cart
//     let cart: any = await Cart.findOne({ userId: req.user?._id });
...
//       cart = await Cart.create({
//         userId: req.user?._id,
//         items: [{ productId, price: product.price, color, size, quantity: 1 }],
//       });
//     } else {
//       // 4. Check if the product with the same color & size exists in the cart
//       const productIndex: number = cart.items.findIndex(
//         (item: IItems) =>
...
//         cart.items[productIndex].quantity += 1;
//       } else {
//         // 6. If it doesn't exist, add it as a new entry
//         cart.items.push({
//           productId,
//           price: product.price,
//           color,
...
//     // 7. Recalculate total price and save
//     calcTotalPriceAtCart(cart);
//     await cart.save();
...
// ! // ! to let the user determine the quantity and color and size but this not complete
...
//         new ApiError(
//           400,
//           "Product ID, color, size, and a valid quantity are required!"
//         )
//       );
//     }
...
//       cart = await Cart.create({
//         userId: req.user?._id,
//         items: [{ productId, price: product.price, color, size, quantity }],
//       });
//     } else {
//       // 4. Check if the same product (with the same color & size) already exists in the cart
//       const productIndex: number = cart.items.findIndex(
//         (item: IItems) =>
//           item.productId.toString() === productId &&
//           item.color === color &&
...
//       if (productIndex > -1) {
//         // 5. If it exists, increment the quantity by the requested amount
